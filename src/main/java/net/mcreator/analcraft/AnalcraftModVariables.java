...
public class AnalcraftModVariables {
	public static class MapVariables extends WorldSavedData {
		public static final String DATA_NAME = "analcraft_mapvars";
		public boolean cumfireVariable = false;
		public MapVariables() {
			super(DATA_NAME);
		}
		...
		public void readFromNBT(NBTTagCompound nbt) {
			cumfireVariable = nbt.getBoolean("cumfireVariable");
		}

		@Override
		public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
			nbt.setBoolean("cumfireVariable", cumfireVariable);
			return nbt;
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				AnalcraftMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				AnalcraftMod.PACKET_HANDLER.sendToAll(new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldVariables extends WorldSavedData {
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				AnalcraftMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				AnalcraftMod.PACKET_HANDLER.sendToDimension(new WorldSavedDataSyncMessage(1, this), world.provider.getDimension());
			}
		}
	...
	public static class WorldSavedDataSyncMessageHandler implements IMessageHandler<WorldSavedDataSyncMessage, IMessage> {
		...
		private void syncData(WorldSavedDataSyncMessage message, MessageContext context, World world) {
			if (context.side == Side.SERVER) {
				...
				if (message.type == 0)
					AnalcraftMod.PACKET_HANDLER.sendToAll(message);
				else
					AnalcraftMod.PACKET_HANDLER.sendToDimension(message, world.provider.getDimension());
			}
			if (message.type == 0) {
